<!--
  Este fichero lo lee Claude Code automaticamente en cada sesion.

  Contenido:
    - El bloque de AVISO de abajo: cuatro conflictos entre esta especificacion y
      el repositorio tal como esta hoy. Pendientes de decision de Pablo.
    - La especificacion de construccion (Pablo, 2026-08-19), integra y sin tocar.
    - El ANEXO A con la regla de proceso anterior (PRD antes de codigo), que
      seguia vigente y no se ha borrado. Copia intacta en
      `docs/design/2026-08-19-regla-de-proceso-prd-anterior.md`.
-->

> ## AVISO — leer antes que nada
>
> **Esta especificacion contradice al repositorio actual en cuatro puntos, y la
> contradiccion NO esta resuelta.** Estan anotados aqui porque el §0.2 de la
> propia especificacion dice «no implementes nada marcado como NO CONSTRUIR; si
> crees que hace falta, pregunta antes». Esto es preguntar.
>
> 1. **«Frontend web» esta en la lista NO CONSTRUIR (§8), y existe.** El
>    repositorio tiene una SPA de ~800 KB en `static/`, y el 2026-08-19 se
>    anadio la vista de tres zonas `/mvp` **por peticion directa de Pablo** (su
>    informe ejecutivo del mismo dia, §4).
> 2. **«Diseno generativo de plantas o distribuciones» esta en NO CONSTRUIR
>    (§8), y es el centro del MVP que Pablo encargo ese mismo dia.** El informe
>    ejecutivo pone «generacion de alternativas» en el nucleo del flujo, y
>    `OP-11` del backlog se revirtio para permitirlo.
> 3. **La estructura de directorios del §3 no es la del repositorio.** La
>    especificacion describe `core/`, `tools/`, `skills/` con Pydantic v2,
>    `mypy --strict` y FastAPI. El repositorio tiene `agente/`, `analyzer/`,
>    `normativa/`, `modelo/`, con dataclasses y Flask, ~900 tests y decisiones
>    de arquitectura ya tomadas y documentadas.
> 4. **El §3 pide codigo y comentarios en ingles.** Todo el repositorio esta en
>    castellano, por una decision deliberada y documentada.
>
> **Lo que NO hago hasta que Pablo lo diga:** ni empezar un `archmuse/core/`
> paralelo, ni congelar la SPA, ni traducir nada. Lo que SI adopto desde ya,
> porque no contradice nada y solo endurece: la regla de oro del §1 (ninguna
> cifra sin procedencia), el `test_no_orphan_numbers` del §13, la prohibicion
> del §7 de afirmar cumplimiento normativo global, y la reconciliacion que no
> adivina del §M1.
>
> Las tres primeras ya son invariantes del repositorio y tienen tests.
...
# ANEXO A — Regla de proceso anterior (seguia vigente el 2026-08-19)

> Esto **no** viene de la especificacion de construccion: es el contenido del
> `CLAUDE.md` que habia antes, conservado porque no lo deroga nada de lo de
> arriba y porque ha estado gobernando el trabajo hasta hoy. Si la
> especificacion lo sustituye, hay que decirlo explicitamente y borrarlo; hasta
> entonces, sigue en vigor.
